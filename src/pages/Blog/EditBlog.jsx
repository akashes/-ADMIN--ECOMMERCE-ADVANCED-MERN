...
import { createBlog, getAllBlogs, getSingleBlog, updateBlog } from "../../features/blog/blogSlice";
...
function isBannerSizeValid(width, height) {
  ...
  const handleSubmit=async(e)=>{
    ...
    //appending file only if it is changed 
    if(file){
    ...
    setLoading(true)
   const resultAction = await dispatch(updateBlog({id:blog._id,formData}))
   console.log(resultAction)
    if(updateBlog.fulfilled.match(resultAction)){
        setLoading(false)
      setFile(null)
      setImage(null)
             showSuccess(resultAction.payload.message || 'Blog Updated successfully')
             setTimeout(() => {
           ...
           if(updateBlog.rejected.match(resultAction)){
            setLoading(false)
             showError(resultAction.payload || 'Failed to Update Blog')
           }
...
useEffect(()=>{
  if(context.isAddProductModalOpen.id){
    dispatch(getSingleBlog(context.isAddProductModalOpen.id))
  }

},[context.isAddProductModalOpen.open,context.isAddProductModalOpen.id])

useEffect(()=>{
  if(blog){
    setFormFields(prev=>({...prev,title:blog.title,description:blog.description}))
    setHtml(blog.description)
    setImage(blog?.blogImage.url)
  }
...
console.log(formFields)
  ...
  value={html} onChange={onChangeDescription} />
        ...
        <div className="w-[250px]">
          <Button disabled={loading || isUploading} type="submit" className={`btn-blue btn-lg mt-3 w-full gap-2 ${loading || isUploading && 'opacity-80'}`}>
            {
                loading ? (
                    <>
                    
                    <CircularProgress size={20}  className=" !text-white" /> Edit Blog
                    </>
                ):(
                    <>
